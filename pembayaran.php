<?php 
  ...
  $sql_buku = "SELECT *, (SELECT COUNT(nama_buku) FROM tb_buku) as jumlah_jenis_buku FROM tb_detail_transaksi
    INNER JOIN tb_buku ON tb_buku.id_buku = tb_detail_transaksi.id_buku
  ...
  $sql_total_harga = mysqli_query($koneksi, "SELECT SUM(harga*qty) as jml_harga FROM tb_buku
  INNER JOIN tb_detail_transaksi ON tb_detail_transaksi.id_buku = tb_buku.id_buku
  ...
  $rows   = [];
  while ($row = mysqli_fetch_array($eksekusi_buku)){
          ...
          <div class="col-md-7 mx-2 p-4 rounded text-white" style="background-color: #005082;">
            <div class="table-responsive">
              ...
              <table class="table text-white">
                ...
                <?php foreach ($rows as $data_buku): ?>
                  <tr>
                    <td><?= $i++; ?></td>
                    <td><?= $data_buku['nama_buku']; ?></td>
                    <td><textarea disabled class="form-control"><?= $data_buku['keterangan']; ?></textarea></td>
                    <td class="text-center"><?= $data_buku['qty']; ?></td>
                    <td>Rp <?= str_replace(",", ".", number_format($data_buku['harga'])); ?></td>
                    <td>Rp <?= str_replace(",", ".", number_format($data_buku['subtotal'])); ?></td>
                  </tr>
                <?php endforeach ?>
              </table>
              <hr style="border: 1px solid">
